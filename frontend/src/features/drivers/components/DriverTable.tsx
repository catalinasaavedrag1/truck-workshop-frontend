  ...
  const columns: TableColumn<Driver>[] = [
    {
      ...
      render: (item) => (
        <div>
          <EntityLink id={item.id} type="driver">
            {item.name}
          </EntityLink>
          <p className="muted-text">{formatRut(item.document) || item.document} - {item.company}</p>
        </div>
      ),
      searchableValue: (item) => getRutSearchText(item.document),
    },
    {
      ...
      render: (item) => {
        ...
        return truck ? (
          <div>
            <EntityLink id={truck.id} type="truck">
              {truck.plate}
            </EntityLink>
  ...
  return (
    <Table
      ...
      getSearchText={(item) => {
        ...
        return [
          item.name,
          getRutSearchText(item.document),
          item.phone,
          item.company,
          item.license,
          truck?.plate || '',
