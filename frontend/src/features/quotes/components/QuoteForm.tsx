...
export function QuoteForm({ workshopCase, onCreated }: QuoteFormProps) {
  ...
  const [quotePreviewTotal, setQuotePreviewTotal] = useState(Number(defaultServiceAmount || 0))
  const [savedMessage, setSavedMessage] = useState('')
  const selectedCustomer = customers.find((customer) =>
    workshopCase.customerId ? customer.id === workshopCase.customerId : customer.name === workshopCase.customerName,
  )
  const quoteReferences: CustomerCreditQuoteReference[] = workshopQuotes.map((quote) => ({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const quote = await createWorkshopQuote({
        ...
        customerId: workshopCase.customerId,
      ...
      form.reset()
      setQuotePreviewTotal(Number(defaultServiceAmount || 0))
    } catch (error) {
  ...
  return (
    <form className="form-grid" onChange={(event) => setQuotePreviewTotal(calculateQuotePreviewTotal(event.currentTarget))} onSubmit={handleSubmit}>
...
function calculateQuotePreviewTotal(form: HTMLFormElement) {
  const formData = new FormData(form)
  const serviceAmount = Number(formData.get('serviceAmount') || 0)
  const laborHours = Number(formData.get('laborHours') || 0)
  const laborRate = Number(formData.get('laborRate') || 0)
  const laborAmount = laborHours > 0 && laborRate > 0 ? laborHours * laborRate : 0

  return Math.max(0, serviceAmount + laborAmount)
}
