...
export function UserRoleForm({ onCancel, onSaved, roles, user }: UserRoleFormProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload: Partial<UserRoleAssignment> = {
  ...
  return (
    <form className={styles.form} key={user?.id || 'new-user'} onSubmit={handleSubmit}>
      ...
      <Input
        ...
        helperText={user ? 'Completar solo si necesitas cambiarla.' : 'Necesaria para iniciar sesion.'}
        label={user ? 'Nueva contrasena' : 'Contrasena inicial'}
        name="password"
        required={!user}
        type="password"
      />
