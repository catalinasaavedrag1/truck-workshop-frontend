...
type ConversationFilter = 'all' | 'open' | 'pending' | 'urgent' | 'unread' | 'whatsapp' | 'email' | 'sent' | 'quotes'
...
const providerOptions = [
  { label: 'Meta WhatsApp Cloud', value: 'whatsapp_cloud' },
  { label: 'Outlook Microsoft Graph', value: 'microsoft_graph' },
]

const deliveryModeOptions = [
  { label: 'Simulacion segura', value: 'simulation' },
  { label: 'Envio real', value: 'live' },
]
...
const relatedEntityOptions = [
  ...
  { label: 'Caso taller', value: 'case' },
  { label: 'Cotizacion taller', value: 'quote' },
  { label: 'Cotizacion flete', value: 'freight-quote' },
...
function providerLabel(provider: CommunicationProvider | 'simulation') {
...
function deliveryModeLabel(mode: CommunicationDeliveryMode) {
  return mode === 'live' ? 'envio real' : 'simulacion'
}

function defaultProviderForChannel(channel: CommunicationChannel): CommunicationProvider {
  return channel === 'whatsapp' ? 'whatsapp_cloud' : 'microsoft_graph'
}

function isProviderForChannel(provider: CommunicationProvider, channel: CommunicationChannel) {
  return channel === 'whatsapp' ? provider === 'whatsapp_cloud' : provider === 'microsoft_graph'
}

function findIntegrationForConversation(
...
function getConversationQuoteReferences(
  conversation: CommunicationConversation,
  quoteLinks: CommunicationQuoteLink[],
...
function isQuoteConversation(conversation: CommunicationConversation, quoteLinks: CommunicationQuoteLink[]) {
  return (
...
function isConversationLinkedToEntity(
  conversation: CommunicationConversation,
  quoteLinks: CommunicationQuoteLink[],
  entityType: string,
  entityId: string,
...
function buildQuoteLinkPayload(
...
function byLastMessageDesc(first: CommunicationConversation, second: CommunicationConversation) {
  return new Date(second.lastMessageAt).getTime() - new Date(first.lastMessageAt).getTime()
}

function bySentAtAsc(first: CommunicationMessage, second: CommunicationMessage) {
  return new Date(first.sentAt).getTime() - new Date(second.sentAt).getTime()
}

function byUpdatedDesc(first: { updatedAt?: string; createdAt?: string }, second: { updatedAt?: string; createdAt?: string }) {
